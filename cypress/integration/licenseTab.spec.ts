...
describe('License Tab Test Cases', () => {
  ...
  it('User Login case', () => {
    ...
    cy.wait('@userData').then((val) => {
      ...
      if(data?.couponId){
        cy.get('[data-cy="license-header"]').should("exist").should('be.visible');
        cy.get('[data-cy="license-text"]').should("exist").should('be.visible');
        cy.contains(`You have signed up with coupon code ${data?.coupon_code?.code} which allowed you free access and you can use the product without restriction. You will receive all future updates for free. For more queries please write us at admin@quicklook.me`);
      }

      else if(data?.allowed_free_access){
        cy.get('[data-cy="license-header"]').should("exist").should('be.visible');
        cy.get('[data-cy="license-text"]').should("exist").should('be.visible');
        cy.contains("You have been given free access and you can use the product without restriction. You will receive all future updates for free. For more queries please write us at admin@quicklook.me");
      }
    });
