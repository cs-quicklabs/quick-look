...
describe("Signup test", function () {
  ...
  let signupDetails: { fname: string; lname: string;  email: string; profileId: string; password: string; confirmPassword: string; verifyEmail: string; };
  before(function () {
  ...
  it("should signup with valid credentials ", () => {
    ...
    cy.xpath('//input[@name="firstName"]').should('be.visible').clear().type(signupDetails.fname);
    cy.xpath('//input[@name="lastName"]').should('be.visible').clear().type(signupDetails.lname);
    cy.xpath('//input[@name="profileId"]').should('be.visible').clear().type(signupDetails.profileId);
  ...
  it('validate verify account', () => {
      ...
      cy.readFile('cypress/fixtures/signUpDetails.json').then((data) => {
         // search for email 
      cy.xpath('//input[@class="ycptinput"]').should('be.visible').type(data.email);
