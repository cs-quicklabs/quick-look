describe('Add SpotLight button test cases', function () {
  ...
  it('validate additional link and add additional link', () => {
    ...
    cy.get('[data-cy="addSpotlightLink"]').type(`${Cypress.env('downloadpath')}`)
    cy.contains('Show spotlight button')
    cy.get('[data-cy="ToggleSpotlight"]').click()
    cy.get('[data-cy="addSpotlightButton"]').click()
    ...
    cy.get('[data-cy="linkText"]').type("abc")
    cy.contains('Add Button link')
    cy.get('[data-cy="linkUrl"]').type("https://www.youtube.com/results?search_query=testqa")
