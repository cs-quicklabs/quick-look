...
export default function Template4 ({ mode,input, loaderData,primaryRestore,secondaryRestore }: any) {
 ...
 const nav = Location.pathname.includes(`${loaderData.username}`)
  ...
  return(
    ...
    <div className={`lg:pl-[0.1rem] flex overflow-auto scrollbar-hide bg-black pb-[3rem] ${nav ?'min-h-[calc(96.5vh+50px)]' : 'min-h-[calc(95.5vh+50px)]'} ${mode ==='mobile' ? 'flex-col' : 'flex-col xl:pt-[3rem] xl:gap-[1rem] xl:flex-row med:gap-[3rem]'}  justify-center bg-no-repeat object-cover overflow-none ${nav? 'lg:gap-[12rem] mediumLaptop:gap-[12rem] largeLaptop:gap-[15rem]' :''}`}>
      <div className={`pt-[1rem] pb-[0.5rem] flex-shrink-0 flex justify-center items-center ${mode ==='mobile' ? 'lg:pb-[0rem] lg:pt-[1.5rem]' : 'sm:mb-0 lg:pb-[0rem] lg:pt-[1.5rem] lg:items-start'} ${nav? '' :''}`} >
        ...
        <img className={`rounded-full object-cover border-[0.3rem] border-white w-[10rem] h-[10rem] sm:w-[13rem] sm:h-[13rem]   ${mode ==='mobile' ? 'lg:w-[20rem] lg:h-[20rem]' : 'lg:w-[20rem] lg:h-[20rem] xl:p-4 mediumLaptop:w-[25rem] mediumLaptop:h-[25rem] largeLaptop:w-[30rem] largeLaptop:h-[30rem]'} ${nav? '' :''} ${loaderData?.profileImage?.secondaryImage || secondaryRestore === true ? '' :''}  `} src={secondaryRestore === true ? defaultimg : loaderData?.profileImage?.secondaryImage} alt='' /> : null}

      </div>

      <div className={`lg:mt-[1.5rem] pl-[1rem] pr-[2rem] w-full ${mode ==='mobile' ? 'lg:pr-[2.5rem] lg:pl-[4rem] xl:pl-[5rem] med:pr-[3.5rem] med:pl-[4rem]' : 'lg:pr-[3rem] lg:pl-[4rem] xl:w-[30rem] med:w-[38rem] medium:w-[45rem] mediumLaptop:w-[50rem] largeLaptop:w-[55rem]'} ${nav ? '' : ''}`}>
