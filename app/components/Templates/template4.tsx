...
export default function Template4 ({ mode,input, loaderData,primaryRestore,secondaryRestore }: any) {
 ...
 const nav = Location.pathname.includes(`${loaderData.username}`)
  ...
  return(
    ...
    <div className={`lg:pl-[0.1rem] flex overflow-auto scrollbar-hide justify-center bg-no-repeat object-cover overflow-none bg-black pb-[3rem] ${nav ?'min-h-[calc(96.5vh+50px)]' : 'min-h-[calc(95.5vh+50px)]'} ${mode ==='mobile' ? 'flex-col' : 'flex-col lg:pt-[3rem] lg:flex-row lg:justify-around'} ${nav? '' :''}`}>
      <div className={`pt-[1rem] pb-[0.5rem] flex-shrink-0 flex justify-center items-center ${mode ==='mobile' ? 'lg:pb-[0rem] lg:pt-[1.5rem]' : 'sm:mb-0 lg:pb-[0rem] lg:pt-[1.5rem] lg:items-start'} ${nav? '' :''}`} >
        ...
        <img loading="lazy" className={`rounded-full object-cover border-[0.3rem] border-white w-[10rem] h-[10rem] sm:w-[13rem] sm:h-[13rem] ${mode ==='mobile' ? 'lg:w-[20rem] lg:h-[20rem] med:w-[23rem] med:h-[23rem] medium:w-[23rem] medium:h-[23rem] mediumLaptop:w-[23rem] mediumLaptop:h-[23rem] largeLaptop:w-[23rem] largeLaptop:h-[23rem]' : 'small:w-[20rem] small:h-[20rem] xl:p-4 med:w-[23rem] med:h-[23rem] medium:w-[23rem] medium:h-[23rem] mediumLaptop:w-[23rem] mediumLaptop:h-[23rem] largeLaptop:w-[23rem] largeLaptop:h-[23rem]'} ${nav? '' :''} ${loaderData?.profileImage?.secondaryImage || secondaryRestore === true ? '' :''}  `} src={secondaryRestore === true ? defaultimg : loaderData?.profileImage?.secondaryImage} alt='profile' /> : null}
      ...
      <div className={`lg:mt-[1.5rem] px-[2rem] w-full ${mode ==='mobile' ? '' : 'xl:pr-0 xl:pl-0 xl:w-[25rem]'} ${nav ? 'SmMedium:w-[30rem] med:w-[35rem] medium:w-[40rem] mediumLaptop:w-[41rem] largeLaptop:w-[48rem]' : mode != 'mobile' ? 'med:w-[30rem] medium:w-[32rem] mediumLaptop:w-[38rem] largeLaptop:w-[45rem]' : ''}`}>
        ...
        {loaderData?.profileInfo?.occupation || input.occupation ||input.location ||loaderData?.profileInfo?.location ?
          <h3 className={`text-gray-50 mt-1.5 text-center  break-normal text-xs leading-5 font-normal lg:text-gray-50 lg:text-2xl lg:leading-8 lg:font-medium ${mode ==='mobile' ? '' : 'lg:text-start xl:w-full'}  ${nav ? 'xl:w-full' : ''} `} >
        ...
        <div className={`lg:flex ${mode ==='mobile' ? '' : ''} ${nav ? '' : ''}`}>
          ...
          <div className={`flex flex-col-reverse ${mode ==='mobile' ? '' : 'lg:flex-col'}`} >
            ...
            <pre className={`text-gray-50 lg:text-gray-50 text-xs leading-5 lg:text-lg lg:leading-none  font-sans flex whitespace-pre-wrap text-justify ${mode ==='mobile' ? '' : ''} ${nav ? '' : ''}`}>
          ...
          <footer className={`flex w-full gap-4 md:gap-8 justify-end  ${nav ? '' : ''} ${ loaderData?.portfolioImage[0] ? 'pt-[1.5rem]' : 'pt-[0rem]'}`}>
