...
export default function Template4({
  ...
  return (
    <>
      ...
      <div
        className={`lg:pl-[0.1rem] flex overflow-auto scrollbar-hide justify-center bg-no-repeat object-cover overflow-none bg-black pb-[3rem] ${nav ? 'min-h-[calc(96.5vh+50px)]' : 'min-h-[calc(95.5vh+50px)]'
          } ${mode === 'mobile' ? 'flex-col' : 'flex-col lg:pt-[3rem] lg:flex-row lg:justify-around'
          } ${nav ? '' : ''}`}
      >
        <div
          className={`pt-[1rem] pb-[0.5rem] flex-shrink-0 flex justify-center items-center ${mode === 'mobile'
        ...
        >
          {secondaryRestore || loaderData?.profileImage?.secondaryImage ? (
            <img
              loading="lazy"
              className={`rounded-full object-cover border-[0.3rem] border-white w-[10rem] h-[10rem] sm:w-[13rem] sm:h-[13rem] ${mode === 'mobile'
        ...
        <div
          className={`lg:mt-[1.5rem] px-[2rem] w-full ${mode === 'mobile' ? '' : 'xl:pr-0 xl:pl-0 xl:w-[25rem]'
            } ${nav
        ...
        >
          <h4
            className={`text-white text-center text-2xl leading-8 font-bold lg:text-4xl lg:leading-10 lg:font-extrabold ml-[0rem] sm:ml-0 ${mode === 'mobile' ? '' : 'lg:text-start'
              }  ${nav ? '' : ''}`}
          >
          ...
          {loaderData?.profileInfo?.occupation ||
            ...
            <h3
              className={`text-gray-50 mt-1.5 text-center  break-normal text-xs leading-5 font-normal lg:text-gray-50 lg:text-2xl lg:leading-8 lg:font-medium ${mode === 'mobile' ? '' : 'lg:text-start xl:w-full'
                }  ${nav ? 'xl:w-full' : ''} `}
            >
          ...
          <div className={`flex flex-col-reverse ${mode === 'mobile' ? '' : 'lg:flex-col'}`}>
            <div
              className={` ${loaderData?.spotlightButton?.toggleSpotlight
                ? 'mt-[1.5rem] xl:mt-[0rem]'
                : 'mt-[2rem]'
                }`}
            >
              <pre
                className={`text-gray-50 lg:text-gray-50 text-xs leading-5 lg:text-lg lg:leading-none  font-sans flex whitespace-pre-wrap text-justify ${mode === 'mobile' ? '' : ''
                  } ${nav ? '' : ''}`}
              >
            ...
            <div className={`pt-8 ${mode === 'mobile' ? 'xl:pb-[1rem]' : 'xl:pt-[2rem]'} `}>
              <div
                className={`flex flex-col ${mode === 'mobile' ? 'flex-col' : 'xl:flex-row xl:gap-[5rem]'
                  } ${nav ? '' : ''}`}
              >
                {loaderData?.profileInfo?.company || input.company ? (
                  <div className={`flex lg:w-[50%] ${mode === 'mobile' ? 'flex-col' : 'flex-col'}`}>
                    <h2
                      className={`text-white lg:text-gray-50 font-medium text-sm leading-5 w-max  ${mode === 'mobile' ? '' : ''
                        }`}
                    >
                      WORK
                    </h2>
                    <h2
                      className={`text-gray-100 lg:text-gray-50 w-max text-sm leading-5 font-normal break-normal ${mode === 'mobile' ? '' : 'xl:w-[65%]'
                        }`}
                    >
                      {input.company}
                    </h2>
                ...
                {loaderData?.profileInfo?.education || input.education ? (
                  <div
                    className={`flex mt-[1.5rem]  ${mode === 'mobile' ? 'flex-col' : 'flex-col xl:mt-[0rem]'
                      } ${nav ? '' : ''}`}
                  >
                    <h2
                      className={`text-white lg:text-gray-50 font-medium text-sm leading-5 w-max ${mode === 'mobile' ? '' : ''
          ...
          <footer
            className={`flex w-full gap-4 md:gap-8 justify-end  ${nav ? '' : ''} ${loaderData?.portfolioImage[0] ? 'pt-[1.5rem]' : 'pt-[0rem]'
              }`}
          >
