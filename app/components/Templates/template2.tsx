  ...
  const nav = Location.pathname.includes(`${loaderData.username}`)
  {
    ...
    return (
      <>
        ...
        <div
          className={`ml-[0.1rem] flex overflow-auto scrollbar-hide justify-center items-center pb-[3rem] pt-[1.5rem] ${
        ...
        >
          ...
          >
            {secondaryRestore || loaderData?.profileImage?.secondaryImage ? (
              <img
                loading="lazy"
                className={`rounded-lg object-cover h-auto w-[16rem] sm:h-[20rem] sm:w-[20rem] md:h-[24rem] md:w-[22rem] ${
                  mode === 'mobile'
                  ...
                  {loaderData?.profileInfo?.education || input.education ? (
                    <div
                      className={`flex mt-[0.75rem] ${
                        mode === 'mobile' ? '' : 'xl:flex-col lg:mt-0'
                      }`}
                    >
                      <h2
                        className={`text-gray-900 font-medium text-base leading-5 w-[1.125rem] mr-[0.5rem] ${
