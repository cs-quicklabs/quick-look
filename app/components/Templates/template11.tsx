...
export default function Template11 ({ mode,input, loaderData,primaryRestore,secondaryRestore }: any) {
 ...
 const nav = Location.pathname.includes(`${loaderData.username}`)
  ...
  return(
    ...
    <div className={`flex overflow-auto scrollbar-hide font-inter bg-white overflow-none pb-[3rem] ${nav ?'min-h-[calc(96.5vh+50px)]' : 'min-h-[calc(95.5vh+50px)]'} ${mode ==='mobile' ? 'flex-col' : 'flex-col xl:gap-[4rem]'} ${nav? '' :''}`} >

      <div className={`flex-shrink-0 flex flex-col relative ${mode ==='mobile' ? 'w-full ' : ''} ${nav? '' :''}`} >
      ...
      <div className=''>
        ...
        <div className={`flex flex-col items-center`}>
          ...
          <div className={`pb-[1rem] w-screen px-[1rem] ${mode ==='mobile' ? 'lg:px-[18rem] xl:px-[25rem] medium:px-[27rem]' : 'xl:hidden'} ${nav ? 'lg:px-[2rem]' : mode !='mobile' ? 'lg:px-[14rem]' : ''}`}>
