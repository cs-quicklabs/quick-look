...
export default function Template7 ({ mode,input, loaderData,primaryRestore,secondaryRestore }: any) {
  ...
  return(
    ...
    <div className={`flex overflow-auto scrollbar-hide ml-[0.1rem] pb-[3rem] flex-col items-center justify-center pt-[1rem] ${nav ?'min-h-[calc(96.5vh+50px)]' : 'min-h-[calc(95.5vh+50px)] '} ${mode ==='mobile' ? 'lg:w-full]' : 'lg:pt-[6rem]'}  justify-center bg-no-repeat object-cover overflow-none` } style={myStyle} >
      ...
      {secondaryRestore || loaderData?.profileImage?.secondaryImage ?
        <img loading="lazy" className={`mb-3 w-60 h-60 rounded-full object-cover ${loaderData?.profileImage?.secondaryImage || secondaryRestore === true ? '' :''} ${nav? ' ' :''} `} src={secondaryRestore === true ? defaultimg : loaderData?.profileImage?.secondaryImage} alt='profile' /> : null}

        <h5 className={`mb-0 lg:mb-[0.375rem] text-gray-800 text-xl lg:text-[36px] leading-8 font-extrabold lg:leading-10 text-center lg:text-4xl`}>
          {loaderData?.firstname} {loaderData?.lastname}
        </h5>

        {loaderData?.profileInfo?.occupation || input.occupation ||input.location ||loaderData?.profileInfo?.location ?
          <h3 className={`text-xs leading-5 lg:text-2xl lg:leading-8 font-normal text-gray-600 w-max break-normal text-center ${mode ==='mobile' ? '' : ''} `} >
      ...
      <div className={`w-[21rem] sm:w-[32rem] ${mode ==='mobile' ? 'lg:w-[25rem] med:w-[32rem] mediumLaptop:w-[38rem] largeLaptop:w-[45rem]' : 'xl:w-[35rem] med:w-[40rem] medium:w-[45rem] mediumLaptop:w-[50rem] largeLaptop:w-[55rem]'} ${nav ? '' : ''}`} >
    ...
    <div className={`mt-[1.5rem] ${mode ==='mobile' ? '' : ''} ${nav ? '' : ''}`}>
        ...
        <div className={`flex items-center justify-center  ${mode ==='mobile' ? '' : ''} ${nav ? '' : ''}`}>
          ...
          <footer className={`flex w-full gap-4 md:gap-8 items-center justify-center ${mode ==='mobile' ? '' : ''}  ${nav ? '' : ''} ${loaderData?.portfolioImage[0] ? 'pt-[1.5rem]' : 'pt-[0rem]'}`}>
