 ...
 const nav = Location.pathname.includes(`${loaderData.username}`)
  ...
  return(
    ...
    <div className={`flex overflow-auto scrollbar-hide bg-black justify-center items-center overflow-none pb-[4rem] ${nav ?'min-h-[calc(96.5vh+50px)]' : 'min-h-[calc(95.5vh+50px)]'} ${mode ==='mobile' ? 'flex-col' : 'flex-col xl:flex-row xl:items-start xl:justify-start med:gap-[5rem] medium:gap-[9rem]'} ${nav? '' :''}`} >
      ...
      <div className={`flex flex-col justify-center items-center w-full pt-[12rem] pl-[1rem] ${mode ==='mobile' ? '' : 'xl:pt-[23rem] lg:w-[35rem] mediumLaptop:w-[40rem] largeLaptop:w-[45rem]'} ${nav ? 'lg:w-[40rem] mediumLaptop:w-[45rem] largeLaptop:w-[50rem]' : ''}`}>
      ...
      <div className={`w-full mt-[-5rem] ${mode ==='mobile' ? '' : 'lg:mt-[-10rem] xl:rounded-none xl:shadow-none'}`} >
          ...
          {loaderData?.profileInfo?.occupation || input.occupation ||input.location ||loaderData?.profileInfo?.location ?
            ...
            <div className={`pb-[1rem] w-full ${mode ==='mobile' ? 'xl:mt-[-1rem]' : 'lg:bg-transparent'} `}>
