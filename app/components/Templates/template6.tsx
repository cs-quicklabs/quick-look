...
export default function Template6({
  ...
  return (
    <>
      ...
      <div
        className={`lg:pl-[0.1rem] flex overflow-auto scrollbar-hide ${nav ? 'min-h-[calc(96.5vh+50px)]' : 'min-h-[calc(95.5vh+50px)]'
          } ${mode === 'mobile' ? 'flex-col' : 'flex-col pt-[3rem] pb-[5rem] lg:gap-[8rem] xl:flex-row'
          }  justify-center bg-no-repeat object-cover overflow-none ${nav ? 'lg:gap-[12rem] mediumLaptop:gap-[12rem] largeLaptop:gap-[15rem]' : ''
          }`}
      >
        <div
          className={`my-4 flex-shrink-0 flex justify-center items-center   ${mode === 'mobile' ? '' : 'sm:mb-0 lg:items-start'
            } ${nav ? '' : ''}`}
        >
          {secondaryRestore || loaderData?.profileImage?.secondaryImage ? (
            <img
              loading="lazy"
              className={`rounded-full object-cover border-4 border-blue-500 w-[10rem] h-[10rem] ${mode === 'mobile'
        ...
        <div
          className={`lg:mt-[1.5rem] pl-[1rem] pr-[2rem] w-full ${mode === 'mobile' ? '' : 'xl:w-[30rem] mediumLaptop:w-[35rem] largeLaptop:w-[40rem]'
            } ${nav ? 'xl:w-[35rem] mediumLaptop:w-[40rem] largeLaptop:w-[45rem]' : ''}`}
        >
          <h4
            className={`text-black text-center  text-2xl leading-8 font-bold lg:text-4xl lg:leading-10 lg:font-extrabold ml-[0rem] sm:ml-0 ${mode === 'mobile' ? '' : 'lg:text-start'
              }  ${nav ? '' : ''}`}
          >
          ...
          {loaderData?.profileInfo?.occupation ||
            ...
            <h3
              className={`text-gray-500 text-center break-normal text-xs leading-5 font-normal lg:text-2xl lg:leading-8 lg:font-medium ${mode === 'mobile' ? '' : 'lg:text-start xl:w-max mediumLaptop:w-full'
                }  ${nav ? 'xl:w-full' : ''} `}
            >
          ...
          <div className="flex flex-col-reverse lg:flex-col">
            <div className="mt-1">
              <pre
                className={`text-gray-700 text-xs leading-5 lg:text-lg lg:leading-none  font-sans flex whitespace-pre-wrap text-justify ${mode === 'mobile' ? '' : ''
                  } ${nav ? '' : ''}`}
              >
            ...
            <div className={`pb-[1rem] lg:pb-[1rem] ${mode === 'mobile' ? '' : ''} `}>
              <div
                className={`flex flex-col pt-4 ${mode === 'mobile' ? 'flex-col' : 'lg:flex-row justify-between'
                  } ${nav ? '' : ''}`}
              >
                {loaderData?.profileInfo?.company || input.company ? (
                  <div className={`flex lg:w-[50%] ${mode === 'mobile' ? '' : ''}`}>
                    <h2
                      className={`text-black font-medium text-base leading-5 w-[1.125rem] mr-[1.18rem] ${mode === 'mobile' ? '' : 'xl:w-[1.525rem]'
                        }`}
                    >
                      <BriefcaseIcon />
                    </h2>
                ...
                {loaderData?.profileInfo?.education || input.education ? (
                  <div
                    className={`flex mt-[1.5rem]  ${mode === 'mobile' ? '' : 'lg:mt-[0rem]'} ${nav ? '' : ''
                      }`}
                  >
                    <h2
                      className={`text-black font-medium text-base leading-5 w-[1.125rem] mr-[1.18rem] ${mode === 'mobile' ? '' : ''
                        }`}
                    >
                      <AcademicCapIcon />
                    </h2>
          ...
          <div
            className={`${loaderData?.portfolioImage.length > 1 ? 'pt-[4rem]' : 'pt-[0rem]'} ${mode === 'mobile' ? '' : ''
              } ${nav ? '' : ''}`}
          >
