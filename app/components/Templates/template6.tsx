...
const myStyle5={
 ...
 `url(${Background6})`,
  ...
  return(
    ...
    <div className={`lg:pl-[0.1rem] flex overflow-auto scrollbar-hide ${nav ?'min-h-[calc(96.5vh+50px)]' : 'min-h-[calc(95.5vh+50px)]'} ${mode ==='mobile' ? 'flex-col' : 'flex-col pt-[3rem] pb-[5rem] lg:gap-[8rem] xl:flex-row'}  justify-center bg-no-repeat object-cover overflow-none ${nav? 'lg:gap-[12rem] mediumLaptop:gap-[12rem] largeLaptop:gap-[15rem]' :''}`}>
      ...
      <div className={`lg:mt-[1.5rem] pl-[1rem] pr-[2rem] w-full ${mode ==='mobile' ? '' : 'xl:w-[30rem] mediumLaptop:w-[35rem] largeLaptop:w-[40rem]'} ${nav ? 'xl:w-[35rem] mediumLaptop:w-[40rem] largeLaptop:w-[45rem]' : ''}`}>
        <h4 className={`text-black text-center  text-2xl leading-8 font-bold lg:text-4xl lg:leading-10 lg:font-extrabold ml-[0rem] sm:ml-0 ${mode ==='mobile' ? '' : 'lg:text-start'}  ${nav ? '' : ''}`}>
          {loaderData?.firstname} {loaderData?.lastname}
        </h4>

        {loaderData?.profileInfo?.occupation || input.occupation ||input.location ||loaderData?.profileInfo?.location ?
          <h3 className={`text-gray-500 text-center break-normal text-xs leading-5 font-normal lg:text-2xl lg:leading-8 lg:font-medium ${mode ==='mobile' ? '' : 'lg:text-start xl:w-max mediumLaptop:w-full'}  ${nav ? 'xl:w-full' : ''} `} >
        ...
        <div className={`mb-10 ${mode ==='mobile' ? '' : ''} ${nav ? '' : ''}`}>
          ...
          <div className='flex flex-col-reverse lg:flex-col'>
            <div className="mt-1">
            <pre className={`text-gray-700 text-xs leading-5 lg:text-lg lg:leading-none  font-sans flex whitespace-pre-wrap text-justify ${mode ==='mobile' ? '' : ''} ${nav ? '' : ''}`}>
                { input?.description?.trim()}
              </pre>
          </div>

          <div className={`pb-[1rem] lg:pb-[1rem] ${mode ==='mobile' ? '' : ''} `}>
              <div className={`flex flex-col pt-4 ${mode ==='mobile' ? 'flex-col' : 'lg:flex-row justify-between'} ${nav ? '' : ''}`} >
                {loaderData?.profileInfo?.company || input.company ?
                  <div className={`flex lg:w-[50%] ${mode ==='mobile' ? '' : ''}`} >
                    <h2 className={`text-black font-medium text-base leading-5 w-[1.125rem] mr-[1.18rem] ${mode ==='mobile' ? '' : 'xl:w-[1.525rem]'}`}>
                    <BriefcaseIcon />
                  </h2>
                ...
                {loaderData?.profileInfo?.education || input.education ?
                  <div className={`flex mt-[1.5rem]  ${mode ==='mobile' ? '' : 'lg:mt-[0rem]'} ${nav ? '' : ''}`} >
                    <h2 className={`text-black font-medium text-base leading-5 w-[1.125rem] mr-[1.18rem] ${mode ==='mobile' ? '' : ''}`} >
                    <AcademicCapIcon />
                  </h2>
